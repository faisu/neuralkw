  ...
  return (
    <section
      ...
      className={`scroll-mt-20 border-t border-border-subtle ${
        reduceMotion ? "" : "h-[280vh] md:h-[360vh]"
      }`}
      aria-label={heading}
    ...
    >
      ...
      >
        <div className="mx-auto flex h-full min-h-0 w-full max-w-[1200px] flex-col px-6 py-3 md:px-10 md:py-4">
          <div className="create-story-frame flex h-full min-h-0 flex-1 flex-col overflow-hidden rounded-lg border border-border-subtle bg-bg-primary shadow-[0_24px_60px_rgba(8,9,11,0.08)] md:grid md:grid-cols-[13.5rem_minmax(0,1fr)] xl:grid-cols-[15rem_minmax(0,1fr)_16.5rem]">
            <aside className="flex shrink-0 flex-col border-b border-border-subtle md:border-r md:border-b-0">
              <div className="border-b border-border-subtle px-4 py-3 sm:px-5">
                ...
                <h2 className="mt-2 text-lg tracking-[-0.03em] text-text-primary">{stage.title}</h2>
                <p className="mt-1 hidden text-sm text-text-muted md:block">{stage.detail}</p>
              </div>
              <ol className="relative flex-1 px-4 py-3 sm:px-5">
                <span
                  className="absolute top-6 bottom-6 left-[1.9rem] w-px bg-bg-surface-deep"
                  aria-hidden="true"
                />
                  ...
                  return (
                    <li key={step.id} className="relative">
                      <button
                        ...
                        className="flex min-h-12 w-full items-start gap-3 py-2 text-left disabled:cursor-default"
                      >
                        <span
                        ...
                        <span className="min-w-0 pt-1">
                          ...
                          <span className="mt-1 block h-px w-16 overflow-hidden bg-bg-surface-deep">
                            <span
              ...
              {mobileAction ? (
                <div className="mt-auto border-t border-border-subtle px-4 py-3 sm:px-5 xl:hidden">
                  {mobileAction}
                </div>
              ) : null}
            </aside>

            <div className="relative min-h-[20rem] flex-1 overflow-hidden md:min-h-0">
